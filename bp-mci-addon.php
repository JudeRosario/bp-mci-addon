	...
	// Hooks into psts_setting_checkout_url and fixes the url
		private function add_hooks () {
			// When a user is updated
				add_action( 'xprofile_updated_profile', array($this,'integrate_xprofile_fields') );

			// When a user signs up or is activated 
				add_action( 'bp_core_signup_user',      array($this,'integrate_xprofile_fields') );
				add_action( 'bp_core_activated_user',   array($this,'integrate_xprofile_fields') );
		}

	// Code to check if the record exists, update otherwise create a new entry 
		
		function integrate_xprofile_fields ($user_id){
			$name = explode(" ", xprofile_get_field_data( 9 , $user_id )) ;
